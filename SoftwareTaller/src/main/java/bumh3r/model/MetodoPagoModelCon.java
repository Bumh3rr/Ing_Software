...
@Getter
public class MetodoPagoModelCon {
    private int id;
    private String metodo;

    public MetodoPagoModelCon(int id, String metodo) {
        this.id = id;
        this.metodo = metodo;
    }
    ...
    public static class ListMetodosPago {
        ...
        private List<MetodoPagoModelCon> list;
        ...
        public List<MetodoPagoModelCon> getList() throws Exception {
            if (this.list == null || this.list.isEmpty()) {
//                this.list = DaoMetodoPago.getListMetodosPagos();
                this.list = List.of(new MetodoPagoModelCon(1,"Efectivo"),
                        new MetodoPagoModelCon(2,"Tarjeta de Crédito"),
                        new MetodoPagoModelCon(3,"Tarjeta de Débito"),
                        new MetodoPagoModelCon(4,"Transferencia Bancaria"),
                        new MetodoPagoModelCon(5,"PayPal"));
                return this.list;
            }
            return this.list;
        }

        public static void addItemsMetodoPago(FlatComboBox<MetodoPagoModelCon> comboBox) {
            PoolThreads.getInstance().getExecutorService().execute(() -> {
                ...
                try {
                    List<MetodoPagoModelCon> list = getInstance().getList();
                    comboBox.removeAllItems();
                    SwingUtilities.invokeLater(() -> {
                        comboBox.setModel(
                                new DefaultComboBoxModel<>(list.stream().toArray(MetodoPagoModelCon[]::new)));
                        comboBox.setRenderer(new ListCellRenderer<MetodoPagoModelCon>() {
                            @Override
                            public Component getListCellRendererComponent(JList<? extends MetodoPagoModelCon> list, MetodoPagoModelCon value, int index, boolean isSelected, boolean cellHasFocus) {
